...
correcao = deg2rad(ones(num_wins,1)*deltaf(df)*(-3.5856));
%correcao = zeros(num_wins,1);
...
% fases
fase_med(1,i:f) = phia(i:f,bin) + correcao;   


% frequência
freq_med(1,i:f) = zeros(1,f);%freq_final_fft(1,i:f);
...
mean_tve(df) = mean(tve);
pos_tve(df) = max(tve - mean_tve(df));
neg_tve(df) = max(mean_tve(df) - tve);
mean_amp_error(df) = mean(amp_error);
pos_amp_error(df) = max(amp_error - mean_amp_error(df));
neg_amp_error(df) = max(mean_amp_error(df) - amp_error);
mean_phase_error(df) = mean(phase_error);
pos_phase_error(df) = max(phase_error - mean_phase_error(df));
neg_phase_error(df) = max(mean_phase_error(df) - phase_error);

phase_error_freq = [phase_error_freq phase_error];
amp_error_freq = [amp_error_freq amp_error];

end

coef = polyfit(deltaf',mean_phase_error',1)

figure
% subplot(3,1,1), hold on, grid on
% errorbar(deltaf,mean_tve,neg_tve,pos_tve)
% ylabel('error (%)'),xlabel('\Deltaf (Hz)'),title('tve error')
subplot(2,1,1), hold on, grid on
errorbar(deltaf,mean_amp_error,neg_amp_error,pos_amp_error)
ylabel('error (%)'),xlabel('\Deltaf (Hz)'),title('amplitude error')
subplot(2,1,2), hold on, grid on
errorbar(deltaf,mean_phase_error,neg_phase_error,pos_phase_error)
ylabel('error (deg)'),xlabel('\Deltaf (Hz)'),title('phase error')


figure
subplot(2,1,1), hold on, grid on
plot(amp_error_freq)
ylabel('error (%)'),xlabel('measures'),title('amplitude error')
subplot(2,1,2), hold on, grid on
plot(phase_error_freq)
ylabel('error (deg)'),xlabel('measures'),title('phase error')
...
% histogram(amp_error(1,:)), title('DFT')
% subplot(1,2,2), hold on, grid on
% histogram(y), title('vmrand')
...
title(strcat(['phase dispersion, with f = ',num2str(freqs(1)),' Hz'])), ylabel('probability'), xlabel('error (deg)')
